...
def build_model():
    encoder_input = Input(shape=(params.config.time_step, params.config.input_dim), name='encoder_input')

    # encoder
    rnn1 = Bidirectional(GRU(params.config.rnn_dim, return_sequences=True), name='rnn1')(encoder_input)  # biGRU = 64?*512
    rnn2 = Bidirectional(GRU(params.config.rnn_dim), name='rnn2')(rnn1)  # biGRU =   1*512

    z_mean = Dense(params.config.z_dim, name='z_mean')(rnn2)  # Linear + ReLU = 1*96
    z_log_var = Dense(params.config.z_dim, name='z_log_var')(rnn2)  # Linear + ReLU = 1*96
    ...
    # espacio latente
    z = Lambda(sampling, output_shape=(params.config.z_dim,), name='z')([z_mean, z_log_var])  # z = 1*96
    ...
    decoder_latent_input = Input(shape=params.config.z_dim, name='z_sampling')  # sampling (output de z)

    repeated_z = RepeatVector(params.config.time_step, name='repeated_z_tension')(decoder_latent_input)  # repeat = 64?*96

    rnn1_output = GRU(params.config.rnn_dim, name='decoder_rnn1', return_sequences=True)(repeated_z)  # GRU = 64?*256

    rnn2_output = GRU(params.config.rnn_dim, name='decoder_rnn2', return_sequences=True)(  # GRU = 64?*256
        rnn1_output)
...
def manipulate_latent_space(piano_roll, vector_up_t, vector_high_d, vector_up_down_t,
    ...
    if with_input and piano_roll is not None:
        ...
        z = np.random.normal(size=(1, params.config.z_dim))
