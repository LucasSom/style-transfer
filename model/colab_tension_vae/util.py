...
def result_sampling(rolls):
    ...
    for i in range(num):
        ...
        for step in range(timesteps):
            melody_note = np.argmax(roll[step, :params.config.melody_output_dim])
            melody_start = roll[step, params.config.melody_output_dim] > 0.5
            bass_note = np.argmax(roll[step,
                                  params.config.melody_output_dim + params.config.melody_note_start_dim:
                                  params.config.melody_output_dim
                                  + params.config.melody_note_start_dim
                                  + params.config.bass_output_dim])
            bass_start = roll[step, params.config.melody_output_dim
                              + params.config.melody_note_start_dim
                              + params.config.bass_output_dim] > 0.5
...
def roll_to_pretty_midi(rolls, pm_old, verbose=False):
    ...
    for timestep in range(rolls.shape[0]):
        ...
        if len(bass_pitch) > 0:
            ...
            # set the start
            if bass_pitch != params.config.bass_dim - 1:

                if timestep == 0 or bass_start or rolls[timestep - 1, bass_pitch + params.config.melody_dim + 1] == 0:
...
def setup_musescore(musescore_path=None):
    ...
    if musescore_path is None or not Path(musescore_path).exists():
        print(
            'Warning: Could not find musescore installation. '
