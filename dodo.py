...
def generate_audios(transferred_path, audios_path):
    df_transferred = load_pickle(transferred_path)
    get_midis(df_transferred, audios_path, verbose=1)
    ...
    for model_name in models.values():
        for e_orig in subdatasets:
            for e_dest in subdatasets:
                if e_orig != e_dest:
                    transferred_path = get_transferred_path(e_dest, e_orig, model_name)
                    audios_path = get_audios_path(model_name, e_orig, e_dest)
                    yield {
                        'name': f"{model_name}-{e_orig}_to_{e_dest}",
                        'file_dep': [transferred_path],
                        'actions': [(generate_audios, [transferred_path, audios_path])],
                        # 'targets': [audios_path],
                        'verbosity': 2
                    }
